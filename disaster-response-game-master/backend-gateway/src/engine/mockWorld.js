...
export function step(world) {
  ...
  // Synthetic hazard-drift and blocked-route spawning are stand-ins for the real
  // simulation-engine's physics — skip them once mergeEngineTick() is supplying the real thing.
  if (!world._usingRealEngine) {
...
// Merges a real simulation-engine tick into our locally-owned world object. We only take the
// *environmental* facts the engine is authoritative over (hazards/blocked_routes/terrain) —
// agents, missions, and resources stay ours since the A*/MCTS planner below still needs to own
// their lifecycle until an agent-intelligence service exists. Victims are seeded from the engine
// once per scenario (its terrain-aware placement), then left alone so our own
// detection/assignment/rescue progress on them isn't clobbered every tick.
export function mergeEngineTick(world, engineState) {
  world.grid = engineState.grid;
  world.hazards = engineState.hazards;
  world.blocked_routes = engineState.blocked_routes;

  world._usingRealEngine = true;
  if (world.scenario !== engineState.scenario || !world._seededVictims) {
    world.victims = engineState.victims;
    world.metrics.victims_total = engineState.victims.length;
    world._seededVictims = true;
  }
}
...
export function applyCommand(world, cmd) {
  switch (cmd.type) {
    case "set_scenario": {
      ...
      world._seededVictims = false; // force a reseed from the real engine's placement for the new scenario
      return { ok: true };
    }
    case "pause":
      world.paused = true;
